...
def parse_item(node):
    ...
    # Edited by: body editor, or fall back to last commenter/reviewer
    editor = content.get("editor")
    if editor and editor.get("login"):
        item["edited_by"] = editor["login"]
    else:
        # Fall back: last person who commented or reviewed
        last_editor = ""
        last_time = ""
        for c in content.get("comments", {}).get("nodes", []):
            t = c.get("createdAt", "")
            if t > last_time and c.get("author"):
                last_time = t
                last_editor = c["author"].get("login", "")
        for r in content.get("reviews", {}).get("nodes", []):
            t = r.get("createdAt", "")
            if t > last_time and r.get("author"):
                last_time = t
                last_editor = r["author"].get("login", "")
        item["edited_by"] = last_editor

    item["last_edited_at"] = content.get("lastEditedAt", "")
...
def write_excel(project_title, items, custom_fields, output_path):
    ...
    for row_idx, item in enumerate(items, 2):
        for col_idx, (_, key) in enumerate(fixed, 1):
            val = item.get(key, "")
            if key in ("created_at", "updated_at", "last_edited_at") and val:
                val = val[:10]
